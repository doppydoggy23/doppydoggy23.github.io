...
function downloadWAV() {
    ...
    let myInt16Array = new Int16Array(WAVInfo.processedSamples.length);
    for (let i=0; i<myInt16Array.length; i++) {

        let smpl=WAVInfo.processedSamples[i];
        smpl=Math.floor(smpl*32766); // convert it to 16 bit signed integers
        myInt16Array[i]=smpl;
    }

    // Create a mono wave file, 44.1 kHz, 16-bit
    wav.fromScratch(1, WAVInfo.sampleRate, '16', myInt16Array);
