...
const GlobalConstants = {
    ...
    saveFileName: "MyInstrument.json",
}
...
const GlobalVariables = {
    ...
    WebMIDIOnDialogAlreadyDisplayed: false,
}
...
async function CheckWebMidiClick() {
    ...
    if (!GlobalVariables.WebMIDIOnDialogAlreadyDisplayed) {
        document.getElementById("MIDIDialog").show(); //show dialog informing that you can use midi
        GlobalVariables.WebMIDIOnDialogAlreadyDisplayed=true;
    }
}
...
function SaveButtonClick() {
    ...
    let savingOBJ= retrieveParsedValues();
    ...
    let file = new File([content], GlobalConstants.saveFileName, {type: "text/plain:charset=UTF-8"});
...
function LoadButtonClick() {
    document.getElementById('my_file').onchange = function (e) {
        ...
        reader.onload = readerEvent => {
            ...
            // set all the loaded values
            // return {WFreqs, WAmplitudes, Attack, Decay, Sustain, SustainVolume, Release};
            for (let i=1; i<=loadingOBJ.WFreqs.length; i++) {
                document.getElementById("W"+i+"FInput").value=loadingOBJ.WFreqs[i-1];
                document.getElementById("W"+i+"FRange").value=document.getElementById("W"+i+"FInput").value;
                document.getElementById("W"+i+"Weight").value=loadingOBJ.WAmplitudes[i-1];
            }
            document.getElementById("AttackRange").value=loadingOBJ.Attack;
            document.getElementById("DecayRange").value=loadingOBJ.Decay;
            document.getElementById("SustainDurationRange").value=loadingOBJ.Sustain;
            document.getElementById("SustainVolumeRange").value=loadingOBJ.SustainVolume;
            document.getElementById("ReleaseRange").value=loadingOBJ.Release;
        
            // if Harmonics option selected, adjust the values
            if (document.getElementById("SelectHarmonicsOptionRG").value==2) {
                adjustValuesForHarmonicsInGUI();
            }        
            collectFormDataAndUpdateSignalProcessorValues(); // send the loaded values to the audio signal processor, if it exists
         }
    }
